...
<template>
  <div>
    <InputLabel v-if="label" :for="label.toLowerCase().replace(' ', '_')">
      {{ label }}
    </InputLabel>
    ...
    <Listbox v-model="inputVal" :multiple="multiple">
      <div class="relative">
        ...
        >
          <ListboxOptions
            class="listbox absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm z-50"
            ...
            >
              <li
                :class="[
                  ...
                  'relative cursor-default select-none py-2 pl-10 pr-4 flex items-center gap-4',
                ]"
              >
                <slot v-if="icon" :icon="false" name="icon">
                  <img
                      :src="option.image"
...
import Api from "@/plugins/setup-axios.js";
